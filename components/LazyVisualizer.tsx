...
function VisualizerPlaceholder({ size = 400 }: { size?: number }) {
  return (
    ...
    >
      ...
      <div className="w-24 h-24 rounded-full bg-gradient-to-br from-sky-500/20 to-sky-500/20 animate-pulse" />
    </div>
  );
}
...
function VisualizerLoader({ size = 400 }: { size?: number }) {
  return (
    <div
      ...
      style={{ width: size, height: size }}
    >
      <div className="w-16 h-16 border-2 border-sky-500/30 border-t-sky-500 rounded-full animate-spin" />
    </div>
  );
}
